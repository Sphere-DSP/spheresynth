...
enum class EQDynamicMode {
    Off,
    Compress,   // Reduce gain when signal exceeds threshold
    Expand      // Increase gain when signal exceeds threshold
};
...
enum class EQCharacterMode {
    Clean,      // Pure digital
    Analog,     // Subtle saturation/warmth
    Vintage,    // More aggressive coloration
    Aggressive  // Heavy saturation
};
...
struct EQBandParams {
    ...
    // Dynamic parameters
    double dynamicThreshold = -20.0;  // dB
    double dynamicRatio = 2.0;        // ratio
    double dynamicAttack = 10.0;      // ms
    double dynamicRelease = 100.0;    // ms
    double dynamicRange = 12.0;       // dB
    
    bool operator==(const EQBandParams& other) const {
        return bypass == other.bypass &&
               type == other.type &&
               std::abs(frequency - other.frequency) < 0.001 &&
               std::abs(q - other.q) < 0.0001 &&
               std::abs(gainDb - other.gainDb) < 0.01 &&
               slope == other.slope &&
...
inline float flushDenormal(float value) {
    return (std::abs(value) < static_cast<float>(DENORMAL_THRESHOLD)) ? 0.0f : value;
}
