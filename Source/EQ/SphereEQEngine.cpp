...
namespace Sphere
{
    ...
    void SphereEQEngine::prepare(double newSampleRate, int samplesPerBlock, int numChannels)
    {
    ...
    void SphereEQEngine::setBandParameters(int bandIndex, const BandParameters& params)
    {
    ...
    void SphereEQEngine::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
    {
        juce::ignoreUnused(midiMessages);
        
        // Process through Oversampler wrapper
        oversampler.processBuffer(buffer, [this](juce::AudioBuffer<float>& buf)
        {
            processInternal(buf);
        });
